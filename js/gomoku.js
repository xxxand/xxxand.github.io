...
function alphaBeta(depth, alpha, beta, player, aiPlayer) {
  ...
  if (player === aiPlayer) {
    ...
    for (const { r, c } of candidates) {
      board[r][c] = player;
      if (player === 1 && forbiddenEnabled && isForbidden(r, c)) {
        board[r][c] = 0;
        continue;
      }
...
function aiMove() {
  ...
  setTimeout(() => {
    ...
    for (const { r, c } of top) {
      board[r][c] = aiPlayer;
      if (aiPlayer === 1 && forbiddenEnabled && isForbidden(r, c)) {
        board[r][c] = 0;
        continue;
      }
      const score =
        alphaBeta(AI_SEARCH_DEPTH, -Infinity, Infinity, opp, aiPlayer) +
        (Math.random() - 0.5) * 2;
      board[r][c] = 0;
      if (score > bestScore) {
        bestScore = score;
...
function undoMove() {
  ...
  currentPlayer = aiEnabled
    ? humanPlayer
    : moveHistory.length > 0
      ? moveHistory[moveHistory.length - 1].player === 1
        ? 2
        : 1
      : 1;
  ...
  statusEl.innerHTML =
    `${currentPlayer === 1 ? "黑棋" : "白棋"} <span class="player ${currentPlayer === 1 ? "black" : "white"}"></span> ${currentPlayer === 1 ? "先行" : "落子"}` +
    fbText +
    aiText;
  drawBoard();
}
